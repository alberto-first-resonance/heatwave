...
from pr_files_exporter import get_pr_files_graphql, process_prs
...
class TestPrFilesExporter(unittest.TestCase):
    ...
    @patch('pr_files_exporter.get_pr_files_graphql')
    ...
    @patch('pr_files_exporter.process_prs')
    def test_main_function(self, mock_process):
        """Test the main function with command line arguments"""
        test_args = [
            '--input', self.input_csv_path,
            '--output', self.output_csv_path,
            '--owner', self.owner,
            '--repo', self.repo,
            '--token', self.github_token
        ]
        
        with patch('sys.argv', ['pr_files_exporter.py'] + test_args):
            from pr_files_exporter import main
            main()
